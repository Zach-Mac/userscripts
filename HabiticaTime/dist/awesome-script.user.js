...
const [keyboardMode, setKeyboardMode] = solidJs.createSignal('normal');
const [moveSubMode, setMoveSubMode] = solidJs.createSignal('push');
const [resizeEdge, setResizeEdge] = solidJs.createSignal('end');
...
Object.entries(dayHeights).reduce((acc, [key, value]) => {
...
const colors = {
  customEvent: Color$1.hsl('purple').desaturate(0.6).lighten(1),
  // Purple
  blueEvent: Color$1.hsl(200, 78, 61),
  // Habitica blue
  finishedEvent: Color$1.hsl(264, 6, 54),
  // Habitica gray
  fullMinMix: Color$1('green'),
  selectedBorderColor: Color$1('black')
};
...
function selectEvents(events) {
  for (const event of events) {
    ...
    const newColor = Color$1(originalColor).lighten(0.3).hsl().string();
...
function updateSelectedEvents(container, selectionArea, calendar) {
  ...
  container.querySelectorAll('.fc-event').forEach(el => {
    ...
    const isNowSelected = eventRect.left < clientSelectionArea.right && eventRect.right > clientSelectionArea.left && eventRect.top < clientSelectionArea.bottom && eventRect.bottom > clientSelectionArea.top;
...
function updateSelectionRect(rect, area) {
...
function autoScroll(container, mouseY) {
  const {
...
function setupSelectionHandlers(calendarEl, calendar) {
  ...
  const onMouseDown = e => {
    ...
    const scrollContainer = document.querySelector('.fc-scroller.fc-scroller-liquid-absolute');
    isSelecting = true;
...
function getMinutesAgoString(now, minutes, rounded = true) {
...
function setEventExtendedProp(event, key, value) {
  ...
  if (currentValue[lastKey] !== value) {
    if (keys.length === 1) {
      ...
      Object.entries(event.extendedProps).forEach(([propKey, propValue]) => {
        if (propKey === keys[0]) {
          event.setExtendedProp(propKey, _extends({}, propValue, {
...
const redoStack = [];
let restoring = false;
const [undoCount, setUndoCount] = solidJs.createSignal(0);
const [redoCount, setRedoCount] = solidJs.createSignal(0);
function updateCounts() {
  setUndoCount(undoStack.length);
  setRedoCount(redoStack.length);
}
function snapshotEvents(calendar) {
  return calendar.getEvents().map(e => e.toJSON());
}
function restoreSnapshot(calendar, snapshot) {
  restoring = true;
...
function pushUndo(calendar) {
  undoStack.push(snapshotEvents(calendar));
  if (undoStack.length > MAX_HISTORY) undoStack.shift();
  redoStack.length = 0;
  updateCounts();
}
function undo(calendar) {
...
function catchupEvents(calendar, finishedMode = 'move') {
  ...
  const hasPastEvent = group => group.some(e => e.start.getTime() < nowMs);
  ...
  if (hasFinishedAfterNow) {
    const finishedAfterNowGroups = buildOverlapGroups(finishedAfterNow.sort((a, b) => a.start.getTime() - b.start.getTime()));
    ...
    if (finishedMode === 'cascade') {
      const finishedBeforeNowGroups = buildOverlapGroups(finishedBeforeNow.sort((a, b) => a.start.getTime() - b.start.getTime()));
  ...
  if (hasUnfinishedToMove) {
    ...
    for (const group of unfinishedGroups) {
      ...
      if (hasPastEvent(group)) {
        ...
        const adjustedStart = findPlacementAvoidingSolidPins(placementTime.getTime(), groupDuration, solidPins);
        ...
        if (groupStart < runningEnd) {
          ...
          const adjustedStart = findPlacementAvoidingSolidPins(newStart.getTime(), groupDuration, solidPins);
          shiftCluster(group, new Date(adjustedStart));
...
function squeezeEvents(calendar) {
  ...
  const nowGroupIdx = clusters.findIndex(c => getClusterStart(c) <= nowMs && getClusterEnd(c) >= nowMs);
...
function findClosestEventToNow(events) {
...
function handleSelectModeClick(event) {
  focusEvent(event.id);
  toggleSelectFocused();
}
...
function getCalendarBounds() {
  ...
  return {
    minMs: base + parseTime(minStr),
    maxMs: base + parseTime(maxStr)
  };
}
...
function movePush(direction, overlap) {
  if (!state.calendar) return;
...
function moveResizeWithPush(direction) {
  if (!state.calendar) return;
  const selected = getTargetEvents();
  ...
  if (startEdge) {
    ...
    const eventsAbove = others.filter(e => e.end.getTime() <= target.start.getTime() + FIVE_MIN).sort((a, b) => b.end.getTime() - a.end.getTime());
    ...
    const eventsBelow = others.filter(e => e.start.getTime() >= target.end.getTime() - FIVE_MIN).sort((a, b) => a.start.getTime() - b.start.getTime());
...
// --- Move mode ---
{
  ...
  handler: () => {
    const sub = moveSubMode();
    if (sub === 'push') movePush(1, false);else if (sub === 'overlap') movePush(1, true);else if (sub === 'resize') moveResize(1);else moveSwap(1);
  }
}, {
  mode: 'move',
  key: 'k',
  ...
  handler: () => {
    if (moveSubMode() === 'resize') setResizeEdge('start');else setMoveSubMode('swap');
  }
}, {
  mode: 'move',
  key: 'e',
...
function createCalendar(initialEvents) {
  ...
  function getScrollOffsetForTime(msToScroll) {
    const slotMinTimeStr = calendar.getOption('slotMinTime').toString();
    const slotMaxTimeStr = calendar.getOption('slotMaxTime').toString();
    const slotMinTimeMs = parseTime(slotMinTimeStr);
    const slotMaxTimeMs = parseTime(slotMaxTimeStr);
  ...
  function adjustWrapperScroll(mouseTimeBefore, mouseY) {
    if (!wrapperEl) return;
  ...
  function slotDurationZoom(value, mouseY) {
  ...
  function zoom(shouldZoomIn, mouseY) {
    ...
    console.debug('BEFORE zoom', 'currentHeight', currentHeight, 'zoomLevel', state.currZoomLevel, 'minHeight', minHeight, 'lastLevelMinHeight', lastLevelMinHeight, 'heightDelta', heightDelta);
    if (shouldZoomIn) {
      if (currentHeight + heightDelta > lastLevelMinHeight * 8 / 10) {
        console.log('currentHeight', currentHeight, 'heightDelta', heightDelta, 'currentHeight + heightDelta', currentHeight + heightDelta, '(lastLevelMinHeight * 9) / 10', lastLevelMinHeight * 9 / 10);
        slotDurationZoom(-1);
    ...
    if (currentHeight != calendar.getOption('height')) calendar.setOption('height', currentHeight);
    if (mouseY !== undefined) {
  ...
  function addEvent(info) {
    ...
    const eventInput = createEvent({
      title,
      start: (info == null ? void 0 : info.start) || calendar.getDate(),
      end: (info == null ? void 0 : info.end) || undefined,
      backgroundColor: colors.customEvent.hsl().string(),
      borderColor: colors.customEvent.hsl().string(),
      customEvent: true
    });
    if (info != null && info.end) {
  ...
  const throttledZoom = throttle((shouldZoomIn, mouseY) => {
    if (shouldZoomIn) {
      zoom(true, mouseY);
    } else {
      zoom(false, mouseY);
  ...
  const calendarOptions = {
    ...
    eventClick: info => {
      ...
      if (info.event.extendedProps.customEvent) {
        const color = !finished ? colors.finishedEvent.hsl().string() : colors.customEvent.hsl().string();
...
function getEventColor(taskColor, taskFullMinutes) {
  let backgroundColor = Color$1(taskColor);
...
function refreshEventColors(column, events) {
...
function getEventsFromColumn(column) {
  const events = [];
  const taskWrapper = column.querySelectorAll('.task-wrapper');
...
function getSavedHabitElement(column) {
  const taskWrapper = column.querySelectorAll('.task-wrapper');
  for (const task of taskWrapper) {
    const titleText = getTaskTitle(task);
...
function getEventsFromSavedHabitNote(column) {
  const task = getSavedHabitElement(column);
  if (!task) return [];
  const taskNotes = getTaskNotes(task);
  const events = JSON.parse(taskNotes);
  return events;
}
...
function TimeCalc() {
  const [currentTime, setCurrentTime] = solidJs.createSignal('');
  const [totalTime, setTotalTime] = solidJs.createSignal('');
  const [fullTime, setFullTime] = solidJs.createSignal('');
  const [newTime, setNewTime] = solidJs.createSignal('');
  const [newFullTime, setNewFullTime] = solidJs.createSignal('');
  function updateTime() {
    const now = new Date();
  ...
  const currentColor = {};
  const durationColor = {
    color: colors.blueEvent.darken(durationDarken).string()
  };
  const newTimeColor = {
    color: colors.blueEvent.darken(timeDarken).string()
  };
  const fullDurationColor = {
    color: colors.defaultFullMin.darken(durationDarken).string()
  };
  const newFullTimeColor = {
  ...
  return (() => {
    ...
    web.style(_el$2, boxStyle);
    web.insert(_el$4, currentTime);
    web.style(_el$6, boxStyle);
    web.insert(_el$8, totalTime);
    web.style(_el$10, boxStyle);
    web.insert(_el$12, newTime);
    web.style(_el$14, boxStyle);
    web.insert(_el$16, fullTime);
    web.style(_el$18, boxStyle);
    web.insert(_el$20, newFullTime);
    web.effect(_p$ => {
      var _v$ = outerStyle,
        _v$2 = _extends({}, labelStyle, currentColor),
        _v$3 = _extends({}, numberStyle, currentColor),
        _v$4 = numberStyle,
        _v$5 = _extends({}, labelStyle, durationColor),
        _v$6 = _extends({}, numberStyle, durationColor),
        _v$7 = numberStyle,
        _v$8 = _extends({}, labelStyle, newTimeColor),
        _v$9 = _extends({}, numberStyle, newTimeColor),
        _v$10 = numberStyle,
        _v$11 = _extends({}, labelStyle, fullDurationColor),
        _v$12 = _extends({}, numberStyle, fullDurationColor),
        _v$13 = numberStyle,
        _v$14 = _extends({}, labelStyle, newFullTimeColor),
        _v$15 = _extends({}, numberStyle, newFullTimeColor);
      _p$.e = web.style(_el$, _v$, _p$.e);
      _p$.t = web.style(_el$3, _v$2, _p$.t);
      _p$.a = web.style(_el$4, _v$3, _p$.a);
      _p$.o = web.style(_el$5, _v$4, _p$.o);
      _p$.i = web.style(_el$7, _v$5, _p$.i);
      _p$.n = web.style(_el$8, _v$6, _p$.n);
      _p$.s = web.style(_el$9, _v$7, _p$.s);
      _p$.h = web.style(_el$11, _v$8, _p$.h);
      _p$.r = web.style(_el$12, _v$9, _p$.r);
      _p$.d = web.style(_el$13, _v$10, _p$.d);
      _p$.l = web.style(_el$15, _v$11, _p$.l);
      _p$.u = web.style(_el$16, _v$12, _p$.u);
      _p$.c = web.style(_el$17, _v$13, _p$.c);
      _p$.w = web.style(_el$19, _v$14, _p$.w);
      _p$.m = web.style(_el$20, _v$15, _p$.m);
      return _p$;
    }, {
      e: undefined,
      t: undefined,
...
function enableTask(taskClickableArea) {
  taskClickableArea.click();
  setRepeatEveryValue('1');
}
function disableTask(taskClickableArea) {
  taskClickableArea.click();
  setRepeatEveryValue('0');
}
function skipTask(taskClickableArea) {
  taskClickableArea.click();
  decrementStreak();
...
const EditDailiesToggle = () => {
  solidJs.createEffect(() => {
    if (showEditButtons()) {
      if (!showButtonsObserver) {
        ...
        showButtonsObserver = dom.observe(document.body, () => {
          createTaskEditButtons();
        });
      }
  ...
  return (() => {
    ...
    web.insert(_el$5, () => showEditButtons() ? 'Stop editing dailies' : 'Edit dailies');
    return _el$5;
  })();
};
...
dom.observe(document.body, () => {
  ...
  const handleDeleteCal = () => {
    var _state$calendar;
    (_state$calendar = state.calendar) == null || _state$calendar.destroy();
  ...
  function handleMinTimeChange(e) {
    ...
    (_state$calendar2 = state.calendar) == null || _state$calendar2.setOption('slotMinTime', input.value + ':00');
  }
  function handleMaxTimeChange(e) {
    var _state$calendar3;
    const input = e.target;
  ...
  const Wrapper = () => {
    ...
    return (() => {
      ...
      _el$3.style.setProperty("margin-bottom", "0");
      _el$3.style.setProperty("overflow", "hidden");
      _el$3.style.setProperty("text-overflow", "ellipsis");
      _el$3.style.setProperty("white-space", "nowrap");
      _el$3.style.setProperty("padding", "2px 0");
      _el$4.style.setProperty("margin-left", "auto");
      _el$4.style.setProperty("display", "flex");
      _el$4.style.setProperty("flex-wrap", "wrap");
      _el$5.$$click = handleCreateCal;
      _el$6.$$click = handleDeleteCal;
      _el$7.$$click = handleSaveCal;
      _el$8.$$click = handleLoadCal;
      _el$9.$$click = handleCatchup;
      _el$10.$$click = handleSqueeze;
      _el$11.$$click = () => state.calendar && undo(state.calendar);
      _el$12.$$click = () => state.calendar && redo(state.calendar);
      _el$13.$$click = () => setShowMore(!showMore());
          ...
          return [(() => {
            ...
            web.insert(_el$14, (() => {
              ...
              return () => _c$2() ? (() => {
                ...
                var _c$4 = web.memo(() => moveSubMode() === 'resize');
                return () => _c$4() ? `-- RESIZE (${resizeEdge()}) --` : `-- MOVE (${moveSubMode()}) --`;
              })();
            })());
            return _el$14;
          })(), (() => {
            ...
            _el$16.$$click = () => setLegendHidden(!legendHidden());
            web.insert(_el$16, () => legendHidden() ? '\uf06e' : '\uf070');
            ...
            web.effect(() => web.setAttribute(_el$16, "title", legendHidden() ? 'Show legend (h)' : 'Hide legend (h)'));
            return _el$15;
          })()];
        }
      }), _el$19);
      ...
      web.effect(_p$ => {
        var _v$ = undoCount() === 0,
          ...
          _v$3 = redoCount() === 0,
          _v$4 = redoCount() === 0 ? 0.35 : 1,
          _v$5 = wrapperHeight() + 'px';
...
dom.observe(document.body, () => {
  ...
  if (!document.querySelector('#timeCalcContainer')) {
    ...
    web.render(() => web.createComponent(TimeCalc, {}), container);
  }
  return true;
});
dom.observe(document.body, () => {
...
dom.observe(document.body, () => {
  ...
  if (window.innerWidth < 1000) {
    const habits = document.querySelector('.tasks-column.habit');
  ...
  // hide rewards column
  const rewards = document.querySelector('.tasks-column.reward');
  ...
  if (dailiesColumn && state.calendar) {
    ...
    for (const eventName of duplicateEvents) {
      ...
      if (dupeEvents()[eventName] !== eventDurations[eventName]) {
        setDupeEvents(_extends({}, dupeEvents(), {
